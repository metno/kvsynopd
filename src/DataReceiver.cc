...
DataReceiver::DataReceiver( App &app_, dnmi::thread::CommandQue &inputQue_,
                            dnmi::thread::CommandQue &outputQue_ ) :
   app(app_), inputQue(inputQue_), outputQue(outputQue_), con(0){
}
...
DataReceiver::doCheckReceivedData( ObsEvent *obsevent ){
   ...
   if( !typeidReceived(*obsevent) ){
      ...
      LOGWARN("No stationid/typeid received!" << "Deleting event: "
            << obsevent->stationInfo()->wmono() << " obstime: "
            << obsevent->obstime());
      delete obsevent;
   }
...
DataReceiver::newData( kvservice::KvObsDataListPtr data ){
   ...
   for( it = data->begin(); it != data->end(); it++ ){
      ...
      LOGINFO("Data received from kvalobs: stationID: " << it->stationid()
            << " (" << station->wmono() << ")" << endl << "       obstime: "
      ...
      if( dataList.size() == 0 ){
         LOGDEBUG("No new data to insert in database:" << endl
               << "  stationid: " << dit->stationID() << " obstime: "
               << dit->obstime() << " typeid: " << dit->typeID() << endl);
         Logger::resetDefaultLogger();
         LOGDEBUG("No new data to insert in database:" << endl
               << "  stationid: " << dit->stationID() << " obstime: "
               << dit->obstime() << " typeid: " << dit->typeID() << endl);
      ...
      errs << "Inserted data # " << dataList.size() << " :" << endl
            << "  stationid: " << dataIt->stationID() << " obstime: "
            << dataIt->obstime() << " typeid: " << dataIt->typeID() << endl;

      LOGINFO(errs.str());

      try{
         event = new ObsEvent(dataIt->obstime(), station);
      ...
      prepareToProcessAnySynopBasedOnThisObs(dataIt->obstime(), station);
      //}
...
void
DataReceiver::prepareToProcessAnySynopBasedOnThisObs(
   ...
   if( maxTime > now )
      maxTime = miutil::miTime(now.year(), now.month(), now.day(), now.hour(),
                               0, 0);
   ...
   while( time <= maxTime ){
      LOGINFO("Posibly regenerating SYNOP for: " << "  wmono: "
            << station->wmono() << " obstime: " << time << endl);
...
bool
DataReceiver::typeidReceived( ObsEvent &event ){
   ...
   ost << "SELECT distinct stationid,typeid FROM data WHERE " << "obstime='"
         << event.obstime().isoTime() << "' AND " << "stationid IN (" << *it;
   ...
   while( retry && now < timeout ){
      ...
      try{
         ...
         LOGERROR("SQLException: cant read the stationid/typeid for " << endl
               << "event: " << ost.str() << " obstime: " << event.obstime()
               << endl << "Reason: " << con->lastError());
         return false;
      }
   }
...
DataReceiver::setDefaultLogger( StationInfoPtr station ){
   try{
      ...
      ost << kvPath("logdir") << "/kvsynop/dr-" << station->wmono() << ".log";
      ...
      LOGERROR("Cant create a logstream for wmono: " << station->wmono());
   }
}
