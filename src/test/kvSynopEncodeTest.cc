...
TEST_F( SynopEncodeTest, RR24_for_RRRtr ) {
  ...
  //cerr << "Station conf 1389: '" << *stInfo << "'\n";
  EXPECT_TRUE(loadSynopDataFromFile("data_7010-1.dat", stInfo, data, validData)) << "Failed to load: data_7010-1.dat";
  EXPECT_TRUE(data.size()>0) << "No data read from: data_7010-1.dat";
  //cerr << "The data: '"<< data <<"' \n";
