...
SynopWorker::newObs(ObsEvent &event)
{
    ...
    if( ! info ) {
        LOGERROR("SynopWorker::newObs: Expecting an StationInfo (info == 0) !");
        return;
    }
    ...
    //We check if this is a event for regenerating a SYNOP
    //due to changed data. If it is, the synop for this time
    //must exist. If it don't exist we could generate
